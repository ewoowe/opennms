...
public class StressCommand implements Action {
    ...
    private EchoRequest buildRequest(Integer messageSize) {
        ...
        if(messageSize > 524288) {
           for(int i=0; i < messageSize/524288; i++) {
               String message = Strings.repeat("*", 524288);
               largeMessage.add(message);
           }
           if(messageSize%524288 != 0) {
               String message = Strings.repeat("*", messageSize%524288);
               largeMessage.add(message);
           }
